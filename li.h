...
public:
    ...
    void push_back(const _Ty &__val)
    {
        if (head == tail)
        {
            ...
            alloc_traits::construct(alloc, head, Node{__val, tail, tail});
        ...
        else
        {
            ...
            alloc_traits::construct(alloc, cur, Node{__val, tail->prev, tail});
            // cur->next = tail;
            // cur->prev = tail->prev;
            tail->prev->next = cur;
            tail->prev = cur;
    ...
    _Ty &back() const { return tail->prev->val; }

    template <typename... Args>
    void emplace_back(Args &&...args)
    {
        Node *new_ptr = alloc_traits::allocate(alloc, 1);
        alloc_traits::construct(alloc, &new_ptr->val, std::forward<Args>(args)...);
        if (head == tail)
        {
            new_ptr->next = tail;
